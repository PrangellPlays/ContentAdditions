...
public class metalscaffolding extends Block implements Waterloggable {
    ...
    public static int calculateDistance(BlockView world, BlockPos pos) {
        ...
        int i = 24;
        if (blockState.isIn(ContentSMPSummonerMod.SCAFFOLDING)) {
            i = blockState.get(DISTANCE);
        } else if (blockState.isSideSolidFullSquare(world, mutable, Direction.UP)) {
            return 0;
        }
        for (Direction direction : Direction.Type.HORIZONTAL) {
            if (!(!(blockState2 = world.getBlockState(mutable.set(pos, direction))).isIn(ContentSMPSummonerMod.SCAFFOLDING) || (blockState2.contains(DISTANCE) && ((i = Math.min(i, blockState2.get(DISTANCE) + 1)) != 1)) || (blockState2.contains(ScaffoldingBlock.DISTANCE) && ((i = Math.min(i, blockState2.get(ScaffoldingBlock.DISTANCE) + 1)) != 1)))) {
                break;
            }
        }
        return i;
